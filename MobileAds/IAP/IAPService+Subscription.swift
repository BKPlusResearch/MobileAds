...
extension IAPService {
    ...
    /// Get subscription info từ UserDefaults
    ...
    func getSubscriptionInfo(for productID: String) -> SubscriptionInfo? {
        return storage.getSubscriptionInfo(for: productID)
    }
    ...
    public func hasActiveSubscription() -> Bool {
        let allProductIDs = storage.getAllProductIDs()

        for productID in allProductIDs {
            if let info = storage.getSubscriptionInfo(for: productID) {
    ...
    func updateSubscriptionStatus(from transaction: Transaction) async {
        // Consumables are not subscriptions — skip saving to storage
        guard transaction.productType != .consumable else {
            print("ℹ️ Skipping storage save for consumable: \(transaction.productID)")
            return
        }
        ...
        storage.saveSubscriptionInfo(subscriptionInfo, for: productID)
    ...
    func checkSubscriptionExpiry(for productID: String) -> Bool {
        guard let info = storage.getSubscriptionInfo(for: productID) else {
        ...
        // No expiry date means it's either non-consumable or non-renewable
    ...
    public func refreshSubscriptionStatus(for productID: IAPProductIdentifiable) async throws {
        ...
        if let transaction = await getCurrentEntitlement(for: productID.productIDString) {
            ...
            if var info = storage.getSubscriptionInfo(for: productID.productIDString) {
                info.status = .expired
                info.lastUpdateDate = Date()
                storage.saveSubscriptionInfo(info, for: productID.productIDString)
    ...
    public func getAllActiveSubscriptions() async -> [SubscriptionInfo] {
        ...
        for await result in Transaction.currentEntitlements {
            do {
                let transaction = try checkVerified(result)
                if let info = storage.getSubscriptionInfo(for: transaction.productID),
                   info.isActive {
